/**
 * Clasa in care implementez comanda touch
		...
		if (!array[1].contains("/")) {
			if (system.curUser.equals("root")
				...
				if (system.curLocation.subordinates.isEmpty()) {
					Entity file = new Entity(true, array[1], "frwx---",system.curUser);
					system.curLocation.add(file);
					return;
				}
				...
				for (Entity ent : system.curLocation.getSubordinates()) {
					if (ent.name.equals(array[1])
						...
						System.out.println("-7: " + helper + ": File already exists");
						return;
					} else if (!ent.name.equals(array[1])
				...
				if (!system.curLocation.rights.substring(5, 6).equals("w")) {
					System.out.println("-5: " + helper + ": No rights to write");
					return;
				}
				if (system.curLocation.subordinates.isEmpty()
		...
		if (array[1].contains("/")) {
			...
			if (tok == 1) {
				...
				if (system.curLocation.name.equals(data[tok - 2][tok - 1])) {
					doit[tok - 1] = comm.getCommand("touch");
